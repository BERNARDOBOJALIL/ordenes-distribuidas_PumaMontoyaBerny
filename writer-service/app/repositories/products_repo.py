...
async def validate_stock(session: AsyncSession, items: list[dict]) -> None:
    ...
    for item in items:
        ...
        if qty <= 0:
            errors.append(
                f"SKU '{sku}': la cantidad debe ser mayor a 0 (recibido: {qty})"
            )
            continue
        ...
        if product.stock <= 0:
            ...
            errors.append(f"SKU '{sku}': solicitado {qty}, disponible {product.stock}")
    ...
    for item in items:
        ...
        if new_stock < 0:
            ...
            warnings.append(
                {
                    "sku": sku,
                    "warning": f"Stock insuficiente (disponible: {product.stock})",
                }
            )
            new_stock = 0
        ...
        logger.info(
            "[reduce_stock] SKU=%s stock %d → %d", sku, product.stock, new_stock
        )
